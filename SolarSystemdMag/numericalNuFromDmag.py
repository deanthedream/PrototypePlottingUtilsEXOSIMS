...
nuss = np.asarray([np.linspace(start=start_stop_points[i],stop=start_stop_points[i+1],num=50,endpoint=True) for i in np.arange(len(start_stop_points)-1)])
...
for i in np.arange(len(nuss)):
    ...
    xrealIsViable = np.logical_and(np.logical_and((np.abs(xreal.imag) < 1e-6),(xreal.real >= nuss[i][0])),(xreal.real <= nuss[i][-1])) #creates 2d array of booleans indicating if imaginary component is small and sol is within viable range
    ...
    quartDict['dmags'] = deltaMag(np.tile(p,(4,1)).T,np.tile(Rp.to('AU'),(4,1)).T,quartDict['d'],quartDict['Phi']) #calculate dmag of the specified x-value

    quartDict['dmagErrors'] = np.abs(quartDict['dmags']-dmag) #calculate error from desired value
    quartDict['smalldmagErrors'] = quartDict['dmagErrors'] < 1e-6 #create boolean array where values are small

    plt.figure(num=i)
    #plt.hist(quartDict['dmagErrors'].flatten(),color='black')
    plt.hist(quartDict['dmags'].flatten(),color='black',bins=50)
    plt.yscale('log')
    plt.title(str(i))
    plt.show(block=False)

    quartDict['xreal'] = xreal #These should be raw quartic solutions
    quartDict['xhasrealInds'] = xhasrealInds #indicies of planets that have a viable solution in this range
...
#planetXsols = np.zeros((len(e),16))#np.zeros((len(e),4))
