...
def probDetectionIsOfType(comp,dmag,uncertainty_dmag,separation,uncertainty_s):
    """
    Args:
        comp (completeness object):
            a completeness object with the EVPOC_hs, count_hs, and count_pop attributes (generated by the subtype completeness module)
        dmag ():
        uncertainty_dmag ():
        separation ():
        uncertainty_s ():
...
for ii,j in itertools.product(np.arange(len(comp.Rp_hi)),np.arange(len(comp.L_lo[0,:]))):
    if comp.count_hs[ii,j] == 0.:
        ...
        prob1[ii,j], normProb1[ii,j] = probDetectionIsOfType(comp,dmag,uncertainty_dmag,separation,uncertainty_s)
