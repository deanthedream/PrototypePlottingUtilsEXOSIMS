...
def probDetectionIsOfType(comp,earth_dmag,uncertainty_dmag,earth_separation,uncertainty_s):
    """
    Args:
        comp (completeness object):
            a completeness object with the EVPOC_hs attribute (generated by the subtype completeness module)
        earth_dmag ():
        uncertainty_dmag ():
        earth_separation ():
        uncertainty_s ():
    Returns:
        ...
        normProb (float):
            prob normalized by the fraction of planets in the sub-pop to the total population
    """
    f_sep = norm(earth_separation,uncertainty_s) #normal probability distribution of the telescope for detected planet separation
    f_dmag = norm(earth_dmag,uncertainty_dmag) #normal probability distribution of the telescope for detected planet=-star delta magnitude
    prob = nquad(lambda si,dmagi:f_sep.cdf(si)*f_dmag.cdf(dmagi)*comp.EVPOCpdf_hs[ii,j].ev(si,dmagi),\
        ranges=[(earth_separation-3.*uncertainty_s,earth_separation+3.*uncertainty_s),\
        (earth_dmag-3.*uncertainty_dmag*earth_dmag,earth_dmag+3.*uncertainty_dmag*earth_dmag)])[0]
    normProb = prob/(comp.count_hs/comp.count_pop)
    return prob, normProb
