...
def probDetectionIsOfType(comp,dmag,uncertainty_dmag,separation,uncertainty_s,sub=-2):
    ...
    if subpop == -2: #whole planet population
        normProbPop = nquad(lambda si,dmagi:f_sep.cdf(si)*f_dmag.cdf(dmagi)*comp.EVPOCpdf_pop.ev(si,dmagi),\
        ...
        normProbPop = nquad(lambda si,dmagi:f_sep.cdf(si)*f_dmag.cdf(dmagi)*comp.EVPOCpdf_earthLike.ev(si,dmagi)/comp.EVPOCpdf_pop.ev(si,dmagi),\
            ranges=[(separation-3.*uncertainty_s,separation+3.*uncertainty_s),\
            (dmag-3.*uncertainty_dmag*dmag,dmag+3.*uncertainty_dmag*dmag)])[0]
        prob = nquad(lambda si,dmagi:f_sep.cdf(si)*f_dmag.cdf(dmagi)*comp.EVPOCpdf_earthLike.ev(si,dmagi),\
            ranges=[(separation-3.*uncertainty_s,separation+3.*uncertainty_s),\
            (dmag-3.*uncertainty_dmag*dmag,dmag+3.*uncertainty_dmag*dmag)])[0]
    else:
        ii = sub[0]
        ...
        normProbPop = nquad(lambda si,dmagi:f_sep.cdf(si)*f_dmag.cdf(dmagi)*comp.EVPOCpdf_hs[ii,j].ev(si,dmagi)/comp.EVPOCpdf_pop.ev(si,dmagi),\
            ranges=[(separation-3.*uncertainty_s,separation+3.*uncertainty_s),\
            (dmag-3.*uncertainty_dmag*dmag,dmag+3.*uncertainty_dmag*dmag)])[0]
...
for ii,j in itertools.product(np.arange(len(comp.Rp_hi)),np.arange(len(comp.L_lo[0,:]))):
    if comp.count_hs[ii,j] == 0.:
        ...
        prob1[ii,j], normProb1[ii,j] = probDetectionIsOfType(comp,dmag,uncertainty_dmag,separation,uncertainty_s,ii,j)
