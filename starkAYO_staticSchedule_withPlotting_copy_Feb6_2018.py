...
class starkAYO_staticSchedule(SurveySimulation):
    ...
    def calc_gaussianFit_Cbydmag(self, sInds, TL, fZ, fEZ, WA, mode, Cb, Csp):
        ...
        for i in np.arange(sInds.shape[0]):
            ...
            fig = plt.figure(999)
            if(i==0):
                plt.plot(dmag[i,:],f[i,:],color='k',label=r'$f_{\Delta mag}(\Delta mag)$')
    ...
    def gaussianTarlorFit(self, x,n,kmax,A,B,C):#Taylor series expansion of the PDF #note that n is depricated and no longer needed
        f = 0
    ...
    def calc_gaussianTaylorFit_Cbydmag(self, sInds, TL, fZ, fEZ, WA, mode, Cb, Csp, popt_saved):
        ...
        plt.xlabel(r'$\Delta$'+'mag',weight='bold')
        plt.ylabel('dCompleteness',weight='bold')
        plt.rc('axes',linewidth=2)
        plt.rc('lines',linewidth=2)
        plt.show(block=False)
        ...
        tmpI = argsort(mu)
        plt.plot(mu[tmpI],taylorCDFnmu[tmpI],color=cmap(0.0),label=r'$\mu$',linewidth=3.0)#most recently added line!,marker='s',alpha=0.5
        plt.plot(mu[tmpI]+sig[tmpI],taylorCDFn1sig[tmpI],color=cmap(0.3333),label=r'$1\sigma$',linewidth=3.0)#most recently added line!,marker='h',alpha=0.5
        plt.plot(mu[tmpI]+2*sig[tmpI],taylorCDFn2sig[tmpI],color=cmap(0.6666),label=r'$2\sigma$',linewidth=3.0)#most recently added line!,marker='D',alpha=0.5
        plt.plot(mu[tmpI]+3*sig[tmpI],taylorCDFn3sig[tmpI],color=cmap(1.0),label=r'$3\sigma$',linewidth=3.0)#most recently added line!,marker='o',alpha=0.5
        cmap = plt.cm.get_cmap('autumn_r')
        ...
        #INSERT FUNCTION THAT USES MY METHOD TO CALCULATE DC/DT######################
        ...
        def func2(t, TL, sInd, fZ, fEZ, WA, mode, Cb, Csp,kmax,A,B,C):#This is the new experimental function 2/6/2018
            x = self.OpticalSystem.calc_dMag_per_intTime(t*u.d, TL, sInd, fZ, fEZ, WA, mode, Cb, Csp)
            n=1 #note that n is depricated and no longer needed
            dC = self.gaussianTarlorFit(x,n,kmax,A,B,C)#input x is dmag (outputs dC/dt)
            ddMag = self.OpticalSystem.ddMag_dt(t*u.d, TL, sInd, fZ, fEZ, WA, mode, Cb, Csp)
            f = (4*1e-3 - (dC*ddMag).to(1/u.d).value[0])**2
            #f = (4*1e-3-self.Completeness.dcomp_dt(t*u.d, TL, sInd, fZ, fEZ, WA, mode, Cb, Csp).to(1/u.d).value[0])**(2)
            return f

        #compare dC/dt functions over a range of t
        #fig = plt.figure(651)
        #tmpt
        ...
        if os.path.isfile(cachefname):#check if file exists
            ...
            for i in np.arange(sInds.shape[0]):
                ...
                if(self.maxdcbydt[i] > 4*1e-3+1e-5):
                    t0[i] = scipy.optimize.golden(func,tol=1e-5,brack=None,args=(TL, sInds[i], fZ[i], fEZ, WA, mode, Cb[i], Csp[i]),full_output=False)#original working version
                    kmax = 100
                    t0[i] = scipy.optimize.golden(func2,tol=1e-5,brack=None,args=(TL, sInds[i], fZ[i], fEZ, WA, mode, Cb[i], Csp[i],kmax,A[i],B[i],C[i]),full_output=False)#new version with gaussians
                if(t0[i]<1e-6):
                    t0[i] = 1e-1
                #print(t0[i])
        #######################################
        ...
        plt.legend(loc=1,prop={'size': 10})
        plt.show(block=False)
        fig.savefig('/home/dean/Documents/SIOSlab/dCbydTandOptimalSelection_new'+'.svg')
        ...
        for j in np.arange(sInds.shape[0]):
            compatt0[j] = self.Completeness.comp_per_intTime(t0[j]*u.d, TL, sInds[j], fZ[j], fEZ, WA, mode, Cb[j], Csp[j])
        ...
        plt.xlim([1e-4,1e1])
        plt.ylim([0,0.14])
        TaylorCDFofGaussianFigsTau.savefig('/home/dean/Documents/SIOSlab/TaylorCDFofGaussianFigsTau_new'+'.svg')
        ...
        #for use determining why C vs Tau plots look off
        #tmpC = np.zeros(intTimes.shape[0])
        #for k in np.arange(intTimes.shape[0]):
        #    tmpC[k] = self.Completeness.comp_per_intTime((t0[j]+1)*u.d, TL, sInds[i], fZ[i], fEZ, WA, mode, Cb[i], Csp[i])
        superTtimeDELUX = np.zeros([651])+1000
        superDMAGdelux = self.OpticalSystem.calc_dMag_per_intTime(superTtimeDELUX*u.d,TL,sInds,fZ,fEZ+np.zeros([651]),WA+np.zeros([651]),mode,Cb,Csp)
        print(saltyburrito)
        plt.close("all")
